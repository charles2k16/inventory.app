<template>
  <div
    ...
    <div class="bg-white rounded-lg shadow-xl max-w-md w-full max-h-screen flex flex-col">
      <div class="p-6 border-b border-gray-200">
        <h3 class="text-lg font-medium text-gray-900">Update Stock</h3>
        <p v-if="product?.itemName" class="text-sm text-gray-500">
          {{ product.itemName }}
        </p>
      </div>
      ...
      <div class="p-6 space-y-4 overflow-y-auto flex-1">
        ...
        <div class="bg-blue-50 p-4 rounded-lg">
          ...
          <p
            :class="[
              'text-2xl font-bold',
              formData.action === 'add' ? 'text-green-600' : 'text-red-600',
            ]">
            {{
              formData.action === 'add'
